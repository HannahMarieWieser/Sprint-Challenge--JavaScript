...
const contactInfo = [];
for (let i = 0; i < graduates.length; i++){
  contactInfo.push([graduates[i].first_name, graduates[i].email].join(' '));
}
...
zooAnimals.filter(function(count){
  if (count.population < 5){
    lowerPopulation.push(count.animal_name);
  }
});
...
*/
let populationTotal = [];
populationTotal = zooAnimals.reduce(function(total, amount){
